...
DO_LOGGING = True
LOG_FILE_NAME = ROOT_DIR + "TEDLog.txt"
...
rssFeeds		= {'TED Talks' : 'http://feeds.feedburner.com/TEDTalks_video'}
...
class DiggNationViewer(xbmcgui.Window):
	def __init__(self):
		...
		self.shows		= xbmcgui.ControlList(40,150,1,1)
		self.itemList		= xbmcgui.ControlList(40,150,350,365,'font13','0xFFFFFFFF')
		self.itemDescription	= xbmcgui.ControlTextBox(420,150,200,365,'font13','0xFFFFFFFF')
		
		self.addControl(self.shows)
		self.addControl(self.itemList)
		self.addControl(self.itemDescription)
		#Move left and right.
		self.itemList.controlRight(self.itemDescription)
		self.itemList.controlLeft(self.itemDescription)
		self.itemDescription.controlRight(self.itemList)
		self.itemDescription.controlLeft(self.itemList)
		count = 1.0
		...
		self.changeShow('TED Talks')
		position = self.itemList.getSelectedPosition()
		filename = self.urlsInOrder[position]
		self.itemDescription.reset()
		for item in self.feeds['TED Talks']:
				if(item.getElement("mediaUrl") == filename):
					self.itemDescription.setText(item.getElement("description"))
